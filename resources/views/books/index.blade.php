...
@section('content')
    ...
    @if(count($books) > 0)
        <table class="table table-striped table-bordered">
            <thead>
            <tr>
                @if(!Auth::guest())<th> id</th>@endif
                <th style=""> @if(!Auth::guest())cover @endif</th>
                <th> title</th>
                <th> author</th>
                <th> genre</th>
                <th> publish date</th>
            ...
            @foreach($books as $book)
                <tr>
                    @if(!Auth::guest())<td> {{$book->id}} </td>@endif
                    <td><a href="{{route('books.show', $book->id)}}"><img src="/storage/cover_images/{{$book->cover_image}}" style="width: 50px; height: 75px;"></a></td>
                        <td style=""> <a href="{{route('books.show', $book->id)}}"> {{$book->title}}</a></td>
